...
#include<fcntl.h>
int     fd_stub;
int write_to_stub(char buff[],size_t length,int fd_stub,int b_offset,
int e_offset);
...
/* Test case to test return value of search stub hash*/
...
/* Test case to test return value of write to stub*/
static void test_write_to_stub(void **state) 
{ 
        
        fd_stub=open("Dedup_new.txt",O_APPEND|O_CREAT|O_RDWR);
        assert_int_equal(write_to_stub("b8c1b4fe198a7cd551c6baa9ef31136627d027b2",
        1000,fd_stub,1000,1999),0);
        close(fd_stub);
